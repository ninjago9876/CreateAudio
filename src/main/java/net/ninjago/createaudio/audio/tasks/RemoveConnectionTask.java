...
public class RemoveConnectionTask extends AudioNetworkTask {
    private final AudioInputLocation toLocation;
    private final AudioOutputLocation fromLocation;

    public RemoveConnectionTask(AudioOutputLocation fromLocation, AudioInputLocation toLocation) {
    ...
    protected HashMap<Integer, AudioNetwork> modify(HashMap<Integer, AudioNetwork> input) {
        if (toLocation.networkUid() != fromLocation.networkUid()) {
            CreateAudio.LOGGER.error("Can not remove connection between different networks directly");
            return input;
        }
        try {
            ...
            toNode.detachInput(fromNode.getOutput(fromLocation.outputIndex()), toLocation.inputIndex());
